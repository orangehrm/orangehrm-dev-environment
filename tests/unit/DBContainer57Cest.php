...
//        $I->seeInShellOutput("Uptime");
    ...
    public function checkMySQLConfigurations(UnitTester $I){
        ...
        $I->runShellCommand("docker exec dev_mysql_57 mysql -uroot -p1234 -e \"show global variables like '%event_scheduler%'\"");
        $I->seeInShellOutput("ON");
        $I->runShellCommand("docker exec dev_mysql_57 mysql -uroot -p1234 -e \"show global variables like '%innodb_log_buffer_size%'\"");
        $I->seeInShellOutput("8388608");
        $I->runShellCommand("docker exec dev_mysql_57 mysql -uroot -p1234 -e \"show global variables like '%innodb_buffer_pool_size%'\"");
        $I->seeInShellOutput("2147483648");
        $I->runShellCommand("docker exec dev_mysql_57 mysql -uroot -p1234 -e \"show global variables like 'max_allowed_packet'\"");
